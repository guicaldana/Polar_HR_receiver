...
## 5. Resolução de Problemas Complexos (BLE no Linux)

Durante o desenvolvimento, especialmente em sistemas operacionais Linux usando o stack BlueZ, desafios de estabilidade de conexão são extremamente comuns. Compilamos um histórico das resoluções aplicadas à arquitetura do `polar_worker.py`:

### 5.1. Colisão GATT e a Leitura de Bateria
**Problema:** Inicialmente, a API lia a porcentagem de bateria assim que se conectava, no mesmo milissegundo em que a Polar H10 negociava os parâmetros de *MTU* e *Connection Interval* com o BlueZ. A colisão de chamadas fazia o BlueZ em pânico derrubar a conexão instantaneamente.
**Resolução Estrutural:** Alterou-se a ordem e prioridade de assinaturas. Em cenários instáveis, é preferível aguardar a consolidação do canal antes de requisitar a leitura unária de atributos de bateria (GATT read vs GATT notify).

### 5.2. Loops de Desconexão por Desuso Físico
**Problema:** Cintos da Polar H10 implementam hard-shutdown na transmissão se não houver umidade e condução elétrica detectando os batimentos. A fita corta a conexão de hardware em ~17 segundos em idle.
**Resolução:** Não se trata de código ou bugs. A fita deve obrigatoriamente estar em uso em um tórax com eletrodos umidificados. O worker foi equipado com backoff exponencial para não causar pânico de reconexão quando a fita estiver sobre uma mesa (desativada fisicamente).

### 5.3. Exigência de Bonding (Pareamento)
**Problema:** Firmware Polar exigindo segurança. A fita conectava, enviava exatamente 1 evento HR Measurement e caía.
**Resolução:** A especificação Heart Rate Profile é tecnicamente aberta, mas certas atualizações de segurança das fitas Polar bloqueiam streams não autenticados. Exigiu-se pareamento completo através da CLI do Linux (`bluetoothctl remove <MAC>` seguido de `bluetoothctl pair <MAC>`). Após o bonding, o ping/pong de criptografia passou a validar, e os dados fluíram indefinidamente.
